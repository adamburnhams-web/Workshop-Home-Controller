...
|---|---|---|
| `controller_h` | H controller | TFT_eSPI, RTClib, SD, Adafruit GFX |
...
Additional sim overrides: `set heater_pct <0-100>` / `set heater_pct_clear 0`
Set pump speed sim: `set h_pump_spd <0-100>` / `set h_pump_spd_clear 0`
Stop W solar pump from H: `simWPumpStop` (sets calPumpActive=1, calSolarTargetC=0)
...
Set pump speed sim: `set pump_spd <0-100>` / `set pump_spd_clear 0`
Set heater pct sim: `set heater_pct <0-100>` / `set heater_pct_clear 0`
...
All addresses are filled in (commissioning complete for both controllers).
...
- [ ] Install MAX485 hardware and test RS485 inter-controller link end-to-end
