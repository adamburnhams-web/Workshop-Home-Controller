...
def parse_sct(path, cutoff_ts=None):
    in_step     = False
    step_pct    = None
    skip_left   = 0
    ...
    with open(path, 'r', errors='replace') as fh:
        for raw in fh:
            ...
            # New 9-col format adds w_pump_pct at col 6; event shifts to col 8
            new_fmt = len(row) >= 9
            event = row[8].strip() if new_fmt else (row[7].strip() if len(row) > 7 else '')

            if event == 'STEP_START':
                flush_step(step_pct, step_stable)
                if cutoff_ts is not None:
                    try:
                        if _dt.datetime.strptime(row[0], '%Y-%m-%d %H:%M:%S') >= cutoff_ts:
                            step_pct = None
                            in_step  = False
                            continue
            ...
            if new_fmt:
                w_pmp = to_float(row[6])
                if w_pmp is not None and w_pmp > 0:
                    stable_buf = []
                    continue
...
for fn in sorted(os.listdir(LOG_DIR)):
    if fn.lower().startswith('sct') and fn.lower().endswith('.csv'):
        print(f"Parsing {fn} ...")
        cutoff = SCT_CUTOFFS.get(fn.lower())
        parse_sct(os.path.join(LOG_DIR, fn), cutoff_ts=cutoff)
